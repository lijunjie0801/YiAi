...
//
//    CGFloat rectheight = self.frame.size.height - 20;
//    CGFloat rectY = self.frame.origin.y -20;
//    
//    rectFrame.size.height = rectheight;
//    rectFrame.origin.y = rectY;
//    
//    self.frame = rectFrame;
    ...
    _maxHeight += 4;
    _perHeight = (CGRectGetHeight(self.frame) - [MyAdapter aDapter:30]-20 - _originSize.y)/_maxHeight;
...
-(void)showAnimation{
    ...
    if (_needXandYLine) {
        ...
        layer.strokeColor = (_colorForXYLine==nil?([AppAppearance sharedAppearance].cellLineColor.CGColor):_colorForXYLine.CGColor);
...
//        _maxHeight += 4;
        ...
        for (NSInteger i = 0; i<5; i++) {
            ...
            textLayer.frame = CGRectMake(self.originSize.x - be - 3, CGRectGetHeight(self.frame) - _originSize.y -height-10, be, 15);
    ...
    for (NSInteger i = 0; i<_valueArr.count; i++) {
        ...
        for (NSInteger j = 0; j<arr.count; j++) {
            ...
            //如果传过来的颜色小于数组的颜色，就都显示蓝色
            itemsView.backgroundColor = (UIColor *)(_columnBGcolorsArr.count<arr.count?[AppAppearance sharedAppearance].mainColor:_columnBGcolorsArr[j]);
            [UIView animateWithDuration:1 animations:^{
                ...
                if (finished) {
                    ...
                    CGSize size = [str boundingRectWithSize:CGSizeMake(_columnWidth, MAXFLOAT) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:[MyAdapter fontDapter:14]]} context:nil].size;
                    
                    textLayer.frame = CGRectMake((i * arr.count + j)*_columnWidth + i*_typeSpace+_originSize.x + _typeSpace, CGRectGetHeight(self.frame) - height - _originSize.y -3 - size.height, _columnWidth, size.height+5);
                    
                    textLayer.string = str;
                    
                    textLayer.fontSize = [MyAdapter fontDapter:14];
