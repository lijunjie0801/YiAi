...
#define MONTH ( 1 )
#define YEAR ( 0 )
#define minTimeInterval 631152000*2
...
-(instancetype)initWithDatePickerMode:(DatePickerMode)datePickerMode MinDate:(NSDate *)minDate MaxDate:(NSDate *)maxDate
{
    if (self = [super init])
    {
        ...
        if (maxDate) {
            ...
            [self setMaxDate:[self extractDayDate:datenow]];
...
-(NSDate *)date
{
    ...
    if (self.datePickerMode!=DatePickerYearMode) {
       month = [self.months objectAtIndex:([self selectedRowInComponent:MONTH])];
    }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (self.datePickerMode != DatePickerYearMonthMode&&self.datePickerMode != DatePickerYearMode) {
...
- (void) selectRow:(NSDate *)date {
    ...
    if (self.datePickerMode == DatePickerYearMode) {
        CGFloat day = [self selectDayIndexWithDate:date];
        [self selectRow:day inComponent:YEAR animated:YES];
    }
    if (self.datePickerMode == DatePickerDateMode) {
        CGFloat day = [self selectDayIndexWithDate:date];
        [self selectRow:day inComponent:DAY animated:YES];
