//
//  AddNFCequipmentViewController.m
//  YiAi
//
...
@property(nonatomic, strong)NSString *deviceNum,*changeStr,*firstPro;

@property(nonatomic, strong)UITextField *devicetf,*numtf,*persontf,*addresstf;
...
@implementation AddNFCequipmentViewController
...
-(void)setIntentDic:(NSDictionary *)intentDic{
    self.deviceNum=intentDic[@"param"];
...
-(void)haha:(UITapGestureRecognizer*)tap{
    ...
    _bigBtn=[[UIButton alloc]initWithFrame:CGRectMake(0, 0, kScreen_Width, kScreen_Height+1000)];
...
-(UIView *)setTopView{
    ...
    NSArray *arr=@[self.firstPro,@"请填写设备名称",@"消防泵"];
...
//            }
    ...
    for (int i=0; i<7; i++) {
        if (i==1) {
            ...
            _addresstf.placeholder=@"请填写设备位置";
        ...
        if (i!=1&i!=2&i!=4) {
            ...
            if (i==0) {
                ...
                rightLab.text=@"请选择安装时间";
            }else if (i==6){
                _dueLab=rightLab;
                rightLab.text=@"请选择到期时间";
            }
...
-(UIView *)footerViews
{
    ...
    if (!_footerView) {
        ...
        self.footerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, (10+(WIDTH-20)*156/345)*2+50+(WIDTH-40)/3+[MyAdapter aDapter:61]+30)];
        self.footerView.backgroundColor = [UIColor whiteColor];
        
        self.yuanImg = [[UIImageView alloc] initWithFrame:CGRectMake(10, 10, WIDTH-20, (WIDTH -20)*156/345)];
         self.yuanImg.tag=0;
...
//        [_collectionView addSubview:self.viewlin];
        ...
        [self.senderBtn setTitle:@"完成添加" forState:UIControlStateNormal];
...
-(void)senderButtonAction
{
    ...
    NSDictionary *param = @{@"appKey":appKey,@"proCode":proCode,@"proCodeName":proCodeName,@"nfcCode":nfcCode,@"number":number,@"deviceName":deviceName,@"deviceType":deviceType,@"deviceSecondType":deviceSecondType,@"personPetect":personPetect,@"installTime":installTime,@"dueTime":dueTime,@"status":status,@"nfcFarImg":nfcFarImg,@"nfcNearImg":nfcNearImg,@"remark":nfcRemark,@"nfcDetailImg":nfcDetailImg,@"position":self.addresstf.text};
    [RequestManager postRequestWithURLPath:[URLManager requestURLGenetatedWithURL:KURLAddNFCInfo] withParamer:param completionHandler:^(id responseObject) {
