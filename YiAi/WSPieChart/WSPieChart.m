...
-(void)drawRect:(CGRect)rect{
    ...
    if (_descArr.count==_valueArr.count&&_descArr.count>0) {
        ...
        for (NSInteger i = 0; i<_descArr.count; i++) {
            ...
            [self drawText:[NSString stringWithFormat:@"%@:%ld 占比:%.1f%c",_descArr[i],[_valueArr[i] integerValue],present,'%'] andContext:contex atPoint:P_M(30+self.frame.size.width/2*(i%2), 20*(i/2  )+25+_chartArcLength*2) WithColor:[UIColor blackColor] andTextFontSize:8];
        }
...
- (void)judgeWhitchOneIsNowAngle:(CGFloat )angel andShowPoint:(CGPoint)p{
    ...
    for (NSInteger i = 0; i<_countPreAngeleArr.count-1; i++) {
        ...
        if ([_countPreAngeleArr[i+1] floatValue]>=angel) {
            ...
            WSPieItemsView *itemsView = _layersArr[i];
            
            WSPieItemsView *saveItems = _layersArr[_saveIndex];
            ...
            [UIView animateWithDuration:0.3 animations:^{
                if (weakself.saveIndex==i) {
                    ...
                    if (saveItems.center.x==self.frame.size.width/2) {
                        ...
                        [weakself.showInfoView updateFrameTo:CGRectMake(p.x, p.y, weakself.showInfoView.frame.size.width, weakself.showInfoView.frame.size.height) andBGColor:colors[i%colors.count] andShowContentString:[NSString stringWithFormat:@"%@\n占比:%.1f%c",weakself.descArr[i],present,'%']];
