...
- (void)commonInit {
    ...
    _beginAngle = radiusFromAngle(-180);
...
-(void)updatePointer:(NSInteger)percent withAnimationTime:(CGFloat)animationTime{
    CGFloat angle = percent/100.0 * (M_PI);
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat centerY = slideLength/2;
    // 添加背景轨道
    CGContextAddArc(currentContext, centerX, centerY, (slideLength-_lineWidth)/2, -M_PI, 0, 0);
    // 设置轨道宽度
    ...
    // 进度条轨道
    CGFloat deltaAngle = _progress*M_PI;
    // 根据进度progress的值绘制进度条
    // 注意: 角度需要使用弧度制
    // 设置圆心x, y坐标
...
- (void)setProgress:(CGFloat)progress {
    ...
    [self updatePointer:100 withAnimationTime:3];
    
    if (progress > 1 || progress < 0) return;
     _progress = progress/0.03;
    
    if (self.selectProgressChange) {
        
        self.selectProgressChange([NSString stringWithFormat:@"%.f", _progress*100]);
    }
    
    
//    self.progressLabel.text = [NSString stringWithFormat:@"%.f", progress*100];
//    //    self.progressLabel.font = [UIFont boldSystemFontOfSize:18];
//    self.progressLabel.font = [UIFont fontWithName:@"AkzidenzGroteskBQ-XBdCnd" size:30];
    
    //    NSLog(@"值=====%@",self.progressLabel.text);
