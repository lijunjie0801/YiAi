...
@property (assign, nonatomic) CGFloat limitdown;
        ...
        for (NSInteger i = 0; i < self.yValueArray.count; i++) {
            ...
            if (i == self.yValueArray.count-1) {
                ...
                CGPoint endPoint = CGPointMake((i+1)*self.pointGap, chartHeight -  (endValue.floatValue-self.yMin)/(self.yMax-self.yMin) * chartHeight+topMargin);
                 ...
                 UIColor*aColor; //点的颜色
                NSNumber *Value = self.yValueArray[i];
                CGFloat la = [Value floatValue];
                if (_limitdown>=1000000000&&_limitup>=1000000000) {
                    aColor = [UIColor whiteColor];
                }else{
                    if ( la>_limitup||la<_limitdown) {
                ...
                CGRect strRect = CGRectMake(endPoint.x-strSize.width/2,endPoint.y-strSize.height-5,strSize.width,strSize.height);
                ...
                if (!_isShowLabel) {
                    ...
                    CGRect strRect = CGRectMake(startPoint.x-strSize.width/2,startPoint.y-strSize.height-5,strSize.width,strSize.height);
                    if (i == 0) {
                        self.firstStrFrame = strRect;
                    ...
                    CGPoint startPointUP = CGPointMake(0, chartHeight -  (self.limitup-self.yMin)/(self.yMax-self.yMin) * chartHeight+topMargin);
                    CGPoint endPointUP = CGPointMake(0+self.frame.size.width,chartHeight -  (self.limitup-self.yMin)/(self.yMax-self.yMin) * chartHeight+topMargin);
                    CGPoint startPointDOWN = CGPointMake(0, chartHeight -  (self.limitdown-self.yMin)/(self.yMax-self.yMin) * chartHeight+topMargin);
                    CGPoint endPointDOWN = CGPointMake(0+self.frame.size.width,chartHeight -  (self.limitdown-self.yMin)/(self.yMax-self.yMin) * chartHeight+topMargin);
                    [self drawLine:context
                        startPoint:startPointDOWN
                          endPoint:endPointDOWN
                         lineColor:[UIColor redColor]
                         lineWidth:.5];
                    [self drawLine:context
                        startPoint:startPointUP
                          endPoint:endPointUP
                         lineColor:[UIColor redColor]
                         lineWidth:.5];
    ...
    if(self.isLongPress)
    {
        ...
        if(nowPoint >= 0 && nowPoint < [self.yValueArray count]) {
            ...
            if ([self isPureFloat:[num floatValue]]) {
                [[NSString stringWithFormat:@"数值:%.2f", [num floatValue]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y+15) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor whiteColor]}];
            }
            else {
                [[NSString stringWithFormat:@"数值:%.0f", [num floatValue]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y+15)withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor whiteColor]}];
