...
@interface UntreatedFireViewController ()<UITableViewDataSource, UITableViewDelegate>
{
...
@implementation UntreatedFireViewController
...
-(void)headerRefresh
{
    ...
    NSDictionary *param = @{@"appKey":[AppDataManager defaultManager].identifier,@"alarmType":@"1",@"handleType":@"3",@"page":[NSString stringWithFormat:@"%d",pageNum],@"pageSize":@10,@"proCode":self.procode};
    ...
    [RequestManager postRequestWithURLPath:[URLManager requestURLGenetatedWithURL:KURLGoodList] withParamer:param completionHandler:^(id responseObject) {
        ...
        if ([dic[@"status"] integerValue] ==100) {
            ...
            //首页列表信息
            self.dataArray = [FireModel mj_objectArrayWithKeyValuesArray:dic[@"datas"][@"Alarm"][@"alarmList"]];
    ...
    if ([self.hasMoreStr intValue]>0) {
        ...
        [RequestManager postRequestWithURLPath:[URLManager requestURLGenetatedWithURL:KURLGoodList] withParamer:param completionHandler:^(id responseObject) {
            ...
            if ([dic[@"status"] integerValue] ==100) {
                ...
                //首页列表信息
                NSArray *array = [FireModel mj_objectArrayWithKeyValuesArray:dic[@"datas"][@"indexProcodes"][@"alarmList"]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.fireModel = self.dataArray[indexPath.section];
